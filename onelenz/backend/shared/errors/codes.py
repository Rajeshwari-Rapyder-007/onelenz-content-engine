...
# --- Content Engine ---
CONTENT_ASSET_NOT_FOUND = ErrorCode(
    "CONTENT_ASSET_NOT_FOUND", "Content asset not found", 404
)
CONTENT_ASSET_PROCESSING = ErrorCode(
    "CONTENT_ASSET_PROCESSING",
    "Asset is currently being processed",
    409,
)
CONTENT_DAILY_LIMIT_REACHED = ErrorCode(
    "CONTENT_DAILY_LIMIT_REACHED",
    "Daily asset upload limit reached",
    429,
)
CONTENT_FILE_TOO_LARGE = ErrorCode(
    "CONTENT_FILE_TOO_LARGE", "File exceeds maximum allowed size", 413
)
CONTENT_INVALID_CATEGORY = ErrorCode(
    "CONTENT_INVALID_CATEGORY", "Invalid content category", 400
)
CONTENT_INVALID_URL = ErrorCode(
    "CONTENT_INVALID_URL", "URL is not reachable", 400
)
CONTENT_TOO_MANY_FILES = ErrorCode(
    "CONTENT_TOO_MANY_FILES",
    "Too many files in a single upload",
    400,
)
CONTENT_UNSUPPORTED_FILE_TYPE = ErrorCode(
    "CONTENT_UNSUPPORTED_FILE_TYPE",
    "File type is not supported",
    400,
)
CONTENT_ZIP_INVALID = ErrorCode(
    "CONTENT_ZIP_INVALID", "Invalid or corrupted ZIP file", 400
)
